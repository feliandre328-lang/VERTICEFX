...
DOMAINS_FOR_SSL="" # deixe vazio para rodar certbot depois
...
need_root(){
  [[ "${EUID}" -eq 0 ]] || die "Rode como root: sudo bash deploy_inicial.sh"
}
...
clone_or_update_repo(){
  ...
  if [[ -d "${APP_DIR}/.git" ]]; then
    log "Repo já existe, atualizando branch ${APP_BRANCH}..."
    sudo -u "${APP_USER}" bash -lc "
      cd '${APP_DIR}' &&
      git fetch origin '${APP_BRANCH}' &&
      git checkout '${APP_BRANCH}' &&
      git reset --hard 'origin/${APP_BRANCH}' &&
      git clean -fd -e '.env' -e 'backend_prod_venv'
    "
  else
    # Se APP_DIR já tiver conteúdo sem .git, limpa para permitir clone limpo
    if [[ -n "$(find "${APP_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null || true)" ]]; then
      warn "APP_DIR sem .git detectado. Limpando conteúdo antigo (preserva .env e backend_prod_venv)..."
      find "${APP_DIR}" -mindepth 1 -maxdepth 1 \
        ! -name ".env" \
        ! -name "backend_prod_venv" \
        -exec rm -rf {} +
    fi
  ...
  [[ -d "${APP_DIR}/${BACKEND_DIR}" ]] || die "Backend não encontrado em ${APP_DIR}/${BACKEND_DIR}"
  [[ -d "${APP_DIR}/${FRONTEND_DIR}" ]] || die "Frontend não encontrado em ${APP_DIR}/${FRONTEND_DIR}"
  log "Repo OK em ${APP_DIR}"
}

make_env(){
  if [[ -f "${ENV_FILE}" ]]; then
    warn ".env já existe, mantendo: ${ENV_FILE}"
    return
  fi
...
patch_django_settings_for_env(){
  [[ -f "${DJANGO_SETTINGS_FILE}" ]] || die "Arquivo não encontrado: ${DJANGO_SETTINGS_FILE}"

  if grep -q "VERTICEFX_PROD_OVERRIDES" "${DJANGO_SETTINGS_FILE}"; then
    log "settings.py já contém overrides de produção."
    return
  fi

  log "Aplicando overrides de produção em settings.py..."
...
setup_frontend(){
  ...
  [[ -d "${APP_DIR}/${FRONTEND_DIR}/dist" ]] || die "Build não gerou dist/ em ${FRONTEND_DIR}"
...
EOF
  ...
  [[ -S /run/gunicorn/gunicorn.sock ]] || {
    ...
    die "Gunicorn não criou /run/gunicorn/gunicorn.sock"
  }
  log "Gunicorn OK"
}
...
install_ssl_if_requested(){
  if [[ -z "${DOMAINS_FOR_SSL}" ]]; then
    warn "DOMAINS_FOR_SSL vazio -> sem HTTPS automático."
    return
  fi
...
main(){
  ...
  [[ "${APP_REPO}" != *"SEU_USUARIO/SEU_REPO"* ]] || die "Edite APP_REPO com seu repositório real."

  log "Início do setup completo"
