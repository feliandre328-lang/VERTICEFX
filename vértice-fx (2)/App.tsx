...
const App: React.FC = () => {
  ...
  const [loginStep, setLoginStep] = useState<'roleSelection' | 'credentials' | '2fa'>('roleSelection');
  ...
  const handleLoginRequest = (requestedRole: UserRole) => {
    setSelectedRoleFor2FA(requestedRole);
    // Admin goes straight to 2FA, Client to credentials
    if (requestedRole === 'ADMIN') {
      setLoginStep('2fa');
    } else {
      // For demo, we assume credentials are correct and move to 2FA
      setLoginStep('2fa');
    }
  };
  ...
  const handle2FAVerification = (code: string) => {
    ...
    if (selectedRoleFor2FA && code.length === 6) {
      ...
      // Reset login state for the next session
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 flex font-sans">
      <Sidebar activePage={activePage} role={role} onNavigate={(p) => { setActivePage(p); setIsMobileMenuOpen(false); }} onLogout={handleLogout} />
        ...
        <header className="h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-10 px-4 md:px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="sm:hidden">
              <span className="text-sm font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-emerald-400">VÉRTICE FX</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div className={`h-8 w-8 rounded flex items-center justify-center text-xs font-bold cursor-pointer border ${
              role === 'ADMIN' ? 'bg-emerald-900/30 border-emerald-800 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-300'
            }`}>
              {role === 'ADMIN' ? 'AD' : 'CM'}
            </div>
          </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-sm p-4 animate-fade-in">
            <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="text-slate-400 hover:text-white p-2">Fechar</button>
            </div>
